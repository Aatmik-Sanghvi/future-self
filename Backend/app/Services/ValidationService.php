...
class ValidationService
{
    ...
    public function emailLogsExistsRules()
    {
        return [
            ...
            'email',
            'max:100',
            Rule::exists('user_email_logs', 'email'), // Unique email validation
        ];
    }
    ...
    // type rule for sendotp
    public function sendOTPTypeRules()
    {
        return [
            'required',
            'in:signup,login,forgot-password,reset-password,change-email,resend',
        ];
    }

    public function emailForOTPRules()
    {
        return [
            'required',
            'email',
    ...
    public function usernameUniqueRules(Request $request, $userId = null)
    {
        return [
            'required',
            'max:50',
            Rule::unique('users')->where('username', $request->username)->whereNull('deleted_at')->ignore($userId), // Unique username validation
        ];
    }

    public function loginInputRules()
    {
        return [
            'required',
            'string',
    ...
    // public function emailSocialUniqueRules($social_id = null)
    // {
    //     return [
    //         'required',
    //         'email',
